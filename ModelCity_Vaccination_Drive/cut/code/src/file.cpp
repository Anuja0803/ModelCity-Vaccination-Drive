...
{
	user u;
	fl2.open("Registered_file",ios::in | ios::binary);	//opening file in binary mode
	if(!fl2)
	{
...
{
	fl.open("Vaccine_file",ios::out | ios::binary);	//open file in binary mode
...
{
	admin ad;	
	fl2.open("Vaccine_file",ios::in | ios::binary);	//opening file in binary mode
	if(!fl2)
	{
		cout<<"File can't open"<<endl;
		return EXIT_FAILURE;
